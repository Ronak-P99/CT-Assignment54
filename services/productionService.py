...
def save(production_data):
    try:
        ...
        with Session(db.engine) as session:
            with session.begin():
                new_production = Production(name=production_data['name'], quantity_produced=production_data['quantity_produced'], product_id=production_data['product_id'], date_produced=production_data['date_produced'])
